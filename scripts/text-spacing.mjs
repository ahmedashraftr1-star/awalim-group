  ...
  for (let n; (n = walk.nextNode()); ) {
    ...
    for (const bb of rg.getClientRects()) {
      ...
      if (outX > slack || outY > slack) {
        out.push({ kind: "نصّ-مقصوص", detail: `${sel(host)} داخل ${sel(clip)} — ${Math.round(Math.max(outX, outY))}px خارج · «${t.slice(0, 34)}»` });
        break;
      }
    }
  }
...
if (!findings.length) { console.log("✔ النصّ يحتمل تباعد المعيار وتكبير 200% بلا قصّ ولا فيض"); process.exit(0); }
const uniq = new Map();
